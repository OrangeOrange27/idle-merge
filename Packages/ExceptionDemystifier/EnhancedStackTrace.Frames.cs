...
namespace Demystifier
{
    public partial class EnhancedStackTrace
    {
        ...
        private static bool ShowInStackTrace(MethodBase method)
        {
            ...
            if (type.Namespace.StartsWith("Package.Logger"))
                return false;
            if (type.Namespace.StartsWith("ZLogger"))
                return false;
            if (type.FullName.Contains("ZLogger"))
                return false;
            if (type.Namespace.StartsWith("Cysharp.Threading.Tasks"))
                return false;

            if (type.FullName.Contains("AddCrashlyticsUnityLogHandlerProcessor"))
                return false;
            if (type.Namespace.StartsWith("Microsoft.Extensions.Logging"))
                return false;
            if (type.Namespace.StartsWith("UnityEngine.Logger") || type.Namespace.StartsWith("UnityEngine.Debug"))
                return false;
